# -*- coding:  utf-8 -*
"""
This module is used to learn bfgs algorithm
...
def get_alphA(weight,dataMat,labelMat,dk): 
    ...
    for k in range(1,10):
        ...
        alpha=a
    return alpha
def bfgs(dataMat, labelMat, iterNum=3):
    dataMat=mat(dataMat)
    ...
    for j in range(iterNum):
        ...
        M1=(S0.dot(y0.T))
        M2=(y0.dot(H0).dot(y0.T))/M1
        M3=S0.T.dot(S0)/M1
        M4=(S0.T.dot(y0).dot(H0)+H0.dot(y0.T).dot(S0))/M1* (-1)
        H1=H0+(1+M2)[0,0]*M3+M4
        weight0=weight1
        H0=H1
